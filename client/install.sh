...
CODEX_LOG="$HOME/Library/Logs/agentpad-codex-status-monitor.log"
...
launchctl bootout "gui/$UID/$CODEX_LABEL" 2>/dev/null || true
...
chmod +x "$DST/codex-desktop-status-monitor.py" 2>/dev/null || true
...
<plist version="1.0"><dict>
  ...
  <key>ProgramArguments</key><array>
    <string>$PY</string>
    <string>$DST/codex-desktop-status-monitor.py</string>
  </array>
  <key>RunAtLoad</key><true/>
  <key>KeepAlive</key><true/>
  <key>StandardOutPath</key><string>$CODEX_LOG</string>
  <key>StandardErrorPath</key><string>$CODEX_LOG</string>
</dict></plist>
EOF
launchctl bootstrap "gui/$UID" "$CODEX_PLIST" 2>/dev/null || launchctl load -w "$CODEX_PLIST"
