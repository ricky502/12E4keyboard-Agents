...
当前选中的 Agent 使用原状态色全亮，其余七个 Agent 保留相同颜色和灯效但降低亮度；按任一 Agent 键会立即移动高亮，不会改变蓝/黄/红等任务归属含义。底部功能区随当前 Agent 提示：空闲时亮“语音、新任务”，等待输入时亮“语音、批准”，完成时“新任务”显示暗绿，异常时四键红闪；工作中只保留“语音”白灯。按键命令不因灯效而改变，其中“拒绝”会输入 `/stop` 并回车。
