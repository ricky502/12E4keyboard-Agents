...
当前选中的工作中 Agent 使用原状态色呼吸，其他工作中 Agent 保持同色常亮并降低亮度；其余空闲 Agent 熄灭，选中的空闲 Agent 白色呼吸。按任一 Agent 键会立即移动呼吸灯并记住选择，客户端重启后不会跳回默认键；颜色不会改变蓝/黄/红等任务归属含义。异常仍使用红色闪烁，避免与选择提示混淆。底部功能区随当前 Agent 提示：空闲时亮“语音、新任务”，等待输入时亮“语音、批准”，完成时“新任务”显示暗绿，异常时四键红闪；工作中只保留“语音”白灯。按键命令不因灯效而改变，其中“拒绝”会输入 `/stop` 并回车。
